...
## Caveats
Right now you have to refresh after resizing through the media query (currently only one at 840px). This is the case in order to see the sub menu of each top level menu item. I'd like to be able to have a seamless interaction when resizing the browser but I believe web designers are the only users that are testing for responsiveness. I'm also pulling in [Modernizr](http://www.modernizr.com/) and the latest jQuery. More details to come as I keep testing.